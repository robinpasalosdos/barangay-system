...
async function generateBarangayClearancePDF(record) {
  const browser = await puppeteer.launch({ headless: true });
  try {
    const imageData = await getImageAsBase64(record.faceFileName);

    const template = templates.barangayClearance;
    ...
    const filename = "barangay_clearance.pdf";
...
ipcMain.on("print-barangay-clearance", async (event, record) => {
  await generateBarangayClearancePDF(record);
});
